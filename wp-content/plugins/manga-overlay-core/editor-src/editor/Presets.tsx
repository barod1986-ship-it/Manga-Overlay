...
export function Presets({ api, boot, workId, element, editable, presets, loading, loadError, reload, onChange }: Props) {
  ...
  return <section className="mol-editor-presets" aria-label="الأنماط المحفوظة">
    ...
    <fieldset disabled={busy || loading || !!loadError}>
      <label>النمط<select value={selected?.id ?? 0} onChange={event => { setId(Number(event.target.value)); setMessage(''); }}>
      ...
      <details><summary>حفظ وإدارة الأنماط</summary>
        <label>اسم النمط<input maxLength={100} value={name} onChange={event => setName(event.target.value)} /></label>
        <label>نطاق النمط الجديد<select value={scope} onChange={event => setScope(event.target.value as Preset['scope'])}>
