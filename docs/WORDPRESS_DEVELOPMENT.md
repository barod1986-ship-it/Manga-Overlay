...
## Progressive editor CSP (Core 0.11.0)

`Security/EditorPolicy.php` adds an enforced CSP before editor access is resolved, so editor documents, redirects and denied responses receive protection. `script-src` accepts same-origin files and a fresh 192-bit cryptographic nonce; `script-src-attr` rejects inline event handlers, and eval is not enabled. WordPress script helper filters add that nonce to trusted generated tags, including import maps and the inert JSON bootstrap. The CSP nonce is independent of the WordPress authentication nonce, has request lifetime only, and is never stored. Existing application CSP headers remain in force because the policy is appended.

`frame-ancestors 'self'` and the legacy SAMEORIGIN fallback restrict embedding to the same origin. Objects are disabled; base URLs and form actions stay on the same origin. Referrer-Policy is same-origin. This is a progressive script/framing policy: image, font and inline style behavior stays compatible with the renderer, and self/blob workers remain available to WordPress. It is not a complete site-wide strict CSP, and it does not make untrusted third-party plugins safe. Reader, admin and REST documents retain their own existing behavior.
